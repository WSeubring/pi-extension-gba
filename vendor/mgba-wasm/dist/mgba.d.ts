...
declare namespace mGBA {
  ...
  export interface mGBAEmulator extends EmscriptenModule {
    ...
    /**
     * Vendored by pi-extension-gba.
     *
     * Returns a Uint8Array VIEW (not a copy) over the current framebuffer
     * in RGBA little-endian byte order (width*height*4 bytes). Caller
     * must `.slice()` the result to persist it across emulator ticks.
     *
     * Returns null when no ROM is loaded.
     */
    getPixelBuffer(): Uint8Array | null;

    /**
     * Vendored by pi-extension-gba.
     *
     * Returns the current framebuffer dimensions in pixels plus the row
     * stride in bytes.
     */
    getPixelBufferDimensions(): { width: number; height: number; stride: number };
